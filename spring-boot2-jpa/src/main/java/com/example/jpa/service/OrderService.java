...
public class OrderService {
   ...
   * </pre>
   *
   * Hibernate is deleting all ingredients and re-inserts the 2 which persist. The reason is simple. If you do not
   * implement equals and hashcode correctly, Hibernate has no way to detect which element you have removed or added.
   *
   * IndexColumn and equals implementation
   *
   * Using a Set required implementation of equals and hashcode. Using an index column this is not necessary. Hibernate
   * uses the index column to identify an element of the collection. Using a List and omitting the index column will
   * always lead to the inefficient deletes and reinserts when updating an element. Implementing equals does not help
   * with List.
   *
   * <pre>
   * 如果 Order 对象里使用 List<Coffee>，则一定是先全部删除后全部再插入一遍 list 中的元素，如上面的日志所示。
   * 但正确实现 hashCode/equals 方法之后并使用 Set<Coffee> 来管理依赖关系，则会不会删除原来的元素，直接插入新数据，如下日志所示:
   *
   * 2019-09-10 11:32:50.015 DEBUG --- [http-nio-8080-exec-1] .hib.per.col.AbstractCollectionPersister [1487] : No rows to delete
   * 2019-09-10 11:32:50.015 DEBUG --- [http-nio-8080-exec-1] .hib.per.col.AbstractCollectionPersister [1519] : Inserting rows of collection: [com.example.jpa.model.Order.coffee#1]
   * 2019-09-10 11:32:50.015 DEBUG --- [http-nio-8080-exec-1] org.hib.SQL                              [94] :
   *     insert
   *     into
   *         t_order_coffee
   *         (order_id, coffee_id)
